...
2026-09-04 (b) by bento completion + ambient canvas + scroll reveal — bento now full 7 cells (no empty columns): row-1 [wide library span-2] [downloads] [pool]; row-2 [quality-score cell (4 progress bars: 9/10 animation, 9/10 spec, 8/10 WebP — green; 2/10 screenshot — red #ff6060)] [$0-to-start + badge "3 free downloads"] [username-filter mono demo] [up-in-minutes 3 mini steps]. CountUp duration → 1600ms (GoClip data-cnt). **New `GridBackground.tsx`**: fixed z-0 canvas, 14×9 green grid lines sine-breathing (alpha 0.03±0.018), pulsing intersection dots (pulse > 0.7), 320px radial mouse glow — off under reduced-motion; content wrapped `relative z-[1]`. **Scroll reveal**: `.reveal` (opacity 0, translateY 50px, .8s ease) → `.in` via IntersectionObserver threshold 0.08 on every section (GoClip `.r` pattern), unobserved after fire, reduced-motion renders visible. Pricing header rebuilt GoClip-pill layout (centered): sub 17px → **purple founder pill** (border rgba(192,132,252,.3), DM Mono 12px) → 13px sub → **green referral-code pill** (mono + mini tag). Navbar dropdown: "Back to home" item removed (kept as direct navbar link on app variant only). World pool lowered 100 → **25** (DB UPDATE + schema.sql default).
